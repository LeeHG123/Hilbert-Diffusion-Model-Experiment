...
class FNO(nn.Module):
    ...
    fft_norm : str, optional
        by default 'forward'
    time_embed_type : str, optional
        Type of time embedding to use, by default 'sincos'
    rff_dim : int, optional
        Dimension for RFFT2VEmbedding, by default None
    rff_sigma : float, optional
        Sigma for RFFT2VEmbedding, by default 1.0
    trainable_rff : bool, optional
        Whether RFFT2VEmbedding is trainable, by default False
    """
    def __init__(self, n_modes, hidden_channels,
        ...
        if time_embed_type == 'rff_t2v':
            self.time_embed = RFFT2VEmbedding(
                embed_dim=self.lifting_channels,
                rff_dim=rff_dim,
                sigma=rff_sigma,
                trainable_rff=trainable_rff
            )
        
        Dense = [nn.Linear(self.lifting_channels, self.hidden_channels)]
        Dense[0].weight.data = default_init()(Dense[0].weight.data.shape)
        nn.init.zeros_(Dense[0].bias)
        Dense.append(nn.Linear(self.lifting_channels, self.lifting_channels))
    ...
    def forward(self, x, t):
        ...
        # 시간 임베딩 계산 - 수정된 부분
        if hasattr(self, 'time_embed') and self.time_embed_type == 'rff_t2v':
...
class FNO2(nn.Module):
    ...
    def __init__(self, config, device='cuda'):
        ...
        # 시간 임베딩 설정
        self.time_embed_type = getattr(config.model, 'time_embedding', 'sincos')
        
        if self.time_embed_type == 'rff_t2v':
            rff_dim = getattr(config.model, 'rff_dim', self.lifting_channels // 4)
            rff_sigma = getattr(config.model, 'rff_sigma', 1.0)
            trainable_rff = getattr(config.model, 'trainable_rff', False)
    ...
    def forward(self, x, t, y=None):
        ...
        if hasattr(self, 'time_embed') and self.time_embed_type == 'rff_t2v':
            ...
            # 조건부 임베딩 처리
            if y is not None:
                cemb = self.time_embed(y)  # 동일한 임베딩 모듈 사용
                temb = temb + cemb  # 임베딩 합산
        else:
            # 기존 방식 지원 (하위 호환성)
            temb = get_timestep_embedding(t, self.lifting_channels)
            if y is not None:
        ...
        temb = self.Dense[m_idx](temb)

        m_idx += 1

        temb = self.Dense[m_idx](self.act(temb))
        m_idx += 1
